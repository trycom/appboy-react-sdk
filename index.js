...
var ReactAppboy = {
	...
	submitFeedback: function(email, feedback, isBug, callback) {
		callFunctionWithCallback(
			AppboyReactBridge.submitFeedback,
			[email, feedback, isBug],
			callback,
		);
	},
	...
	setCustomUserAttribute: function(key, value, callback) {
		...
		if (value instanceof Date) {
			callFunctionWithCallback(
				AppboyReactBridge.setDateCustomUserAttribute,
				[key, Math.floor(value.getTime() / 1000)],
				callback,
			);
		} else if (value instanceof Array) {
			callFunctionWithCallback(
				AppboyReactBridge.setCustomUserAttributeArray,
				[key, value],
				callback,
			);
		} else if (valueType === 'boolean') {
			callFunctionWithCallback(
				AppboyReactBridge.setBoolCustomUserAttribute,
				[key, value],
				callback,
			);
			...
			if (parseInt(value) === parseFloat(value)) {
				callFunctionWithCallback(
					AppboyReactBridge.setIntCustomUserAttribute,
					[key, value],
					callback,
				);
			} else {
				callFunctionWithCallback(
					AppboyReactBridge.setDoubleCustomUserAttribute,
					[key, value],
					callback,
				);
			}
	...
	incrementCustomUserAttribute: function(key, value, callback) {
		callFunctionWithCallback(
			AppboyReactBridge.incrementCustomUserAttribute,
			[key, value],
			callback,
		);
	},
	 ...
	 * @param {function(error, result)} callback - A callback that receives the function call result.
	 */
	setPushNotificationSubscriptionType: function(
		notificationSubscriptionType,
		callback,
	) {
		callFunctionWithCallback(
			AppboyReactBridge.setPushNotificationSubscriptionType,
			[notificationSubscriptionType],
			callback,
		);
	},
	 ...
	 * @param {function(error, result)} callback - A callback that receives the function call result.
	 */
	setEmailNotificationSubscriptionType: function(
		notificationSubscriptionType,
		callback,
	) {
		callFunctionWithCallback(
			AppboyReactBridge.setEmailNotificationSubscriptionType,
			[notificationSubscriptionType],
			callback,
		);
	},
	...
	addToCustomUserAttributeArray: function(key, value, callback) {
		callFunctionWithCallback(
			AppboyReactBridge.addToCustomAttributeArray,
			[key, value],
			callback,
		);
	},
	...
	removeFromCustomUserAttributeArray: function(key, value, callback) {
		callFunctionWithCallback(
			AppboyReactBridge.removeFromCustomAttributeArray,
			[key, value],
			callback,
		);
	},
	...
	unsetCustomUserAttribute: function(key, callback) {
		callFunctionWithCallback(
			AppboyReactBridge.unsetCustomUserAttribute,
			[key],
			callback,
		);
	},
	...
	setTwitterData: function(
		...
		if (
			...
			AppboyReactBridge.setTwitterData(
				id,
				screenName,
				name,
				description,
			...
			console.log(
				'Invalid null argument(s) passed to ReactAppboy.setTwitterData(). Twitter data not set.',
			);
		}
	},
	...
	setFacebookData: function(facebookUserDictionary, numberOfFriends, likes) {
		if (numberOfFriends !== null) {
			AppboyReactBridge.setFacebookData(
				facebookUserDictionary,
				numberOfFriends,
				likes,
			);
		} else {
			console.log(
				'Invalid null argument(s) passed to ReactAppboy.setFacebookData(). Facebook data not set.',
			);
		}
	},
	 ...
	 */
	logCardImpression: function(index) {
		AppboyReactBridge.logCardImpression(index);
	},
	 ...
	 * 	 * @param {integer} index - Index of the Card
	 */
	logCardClicked: function(index) {
		AppboyReactBridge.logCardClicked(index);
	},

	/**
	 * Returns the News Feed cards for the given category.
	 * @param {CardCategory} category - Card category. Use ReactAppboy.CardCategory.ALL to get all cards.
	 * @param {function(error, result)} callback - A callback that receives the function call result.
	 * Note that for Android, a successful result relies on a FeedUpdatedEvent being posted at least once. There is also a slight
	 * race condition around calling changeUser, which requests a feed refresh, so the cards returned may not always be accurate.
	 */
	getCardsForCategories: function(category, callback) {
		callFunctionWithCallback(
			AppboyReactBridge.getCardsForCategories,
			[category],
			callback,
		);
	},
	...
	getCardCountForCategories: function(category, callback) {
		callFunctionWithCallback(
			AppboyReactBridge.getCardCountForCategories,
			[category],
			callback,
		);
	},
	...
	getUnreadCardCountForCategories: function(category, callback) {
		callFunctionWithCallback(
			AppboyReactBridge.getUnreadCardCountForCategories,
			[category],
			callback,
		);
	},
